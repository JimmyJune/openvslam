...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
                   ...
                   const float scale, const std::string& map_db_path, const float fps) {
    // camera manager
...
#endif
    if(manager.size()==0){
        spdlog::critical("cannot detect any cameras");
        SLAM.shutdown();
        return;
    }
    SpinCam video(manager.getCamera(cam_num));
...
int main(int argc, char* argv[]) {
    ...
    auto fps = op.add<popl::Value<float>>("f", "fps", "frame rate of camera", 30.0);
    ...
    if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
        mono_tracking(cfg, vocab_file_path->value(), cam_num->value(), mask_img_path->value(),
                      scale->value(), map_db_path->value(), fps->value());
    }
